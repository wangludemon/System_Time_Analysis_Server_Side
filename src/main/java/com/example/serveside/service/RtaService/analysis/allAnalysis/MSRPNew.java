...
/**  MSRP analysis from Priority Assignment on Partitioned Multiprocessor Systems With Shared Resource
 *  Response time including:
 *  WCET
 *  Resource execution time
 *  interference: preemption time : WCET + res exe (without indirect spin)
 *  direct spin Blocking
 *  indirect spin blocking
 *  arrival blocking **/
    ...
    private long[][] busyWindow(ArrayList<ArrayList<SporadicTask>> tasks, ArrayList<Resource> resources, long[][] response_time, boolean btbHit) {
        ...
        for (int i = 0; i < tasks.size(); i++) {
            for (int j = 0; j < tasks.get(i).size(); j++) {
                ...
                long spin_delay = directRemoteDelay(task, tasks, resources, response_time, response_time[i][j], btbHit) ;  // direct_spin + indirect_spin

                task.interference = highPriorityInterference(task, tasks, response_time[i][j], response_time, resources, btbHit);   // wcet+res_exe
                task.local = localBlocking(task, tasks, resources, response_time, response_time[i][j], btbHit);

                response_time_plus[i][j] = task.Ri = task.WCET + spin_delay + task.interference + task.local + task.pure_resource_execution_time;
    ...
    private long directRemoteDelay(SporadicTask t, ArrayList<ArrayList<SporadicTask>> tasks, ArrayList<Resource> resources, long[][] Ris, long Ri,
        ...
        for (int i = 0; i <tasks.get(t.partition).size(); i++) {
            if (tasks.get(t.partition).get(i).priority > t.priority) {
                ...
                // 间接阻塞
                btb_interference += getIndirectSpinDelay(hpTask, Ri, Ris[t.partition][i], Ris, tasks, resources, btbHit);
