...
import com.example.serveside.service.RtaService.analysis.allAnalysis.MSRPNew;
import com.example.serveside.service.RtaService.analysis.allAnalysis.MSRPNewForModeSwitch;
...
@Component
public class MsrpNewAnalyzer implements ResponseTimeAnalyzer {
    @Override
    public AnalysisMethod method() {
        return AnalysisMethod.MSRP_NEW;
    }
    ...
    public long[][] analyze(ArrayList<ArrayList<SporadicTask>> totalTasks,
        ...
        if (mode == SystemMode.LO || mode ==SystemMode.HI){
            ...
            return new MSRPNew().getResponseTime(analyzeTasks, resources, debugPrint);
        }

        else{
            ModeValueApplier.apply(totalTasks, resources, SystemMode.LO);
            // 需要先获取Ri_LO, 才能调用mode switch的分析
            long[][] ri = new MSRPNew().getResponseTime(totalTasks, resources, debugPrint);
            // 如果发现不能调度，不需要继续计算mode switch
            if (!isSchedulable(totalTasks, ri)){
                return null;
            }
            ModeValueApplier.apply(totalTasks, resources, mode);
            return new MSRPNewForModeSwitch().getResponseTime(analyzeTasks, resources, lowTasks, debugPrint);   // 要传入 hitask 和 lotask分开传入
        }
    }
